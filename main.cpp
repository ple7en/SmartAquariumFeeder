...
//SPIFFS.format();
  ...
  String S = blynk_token;
    ...
    if (!wifiManager.autoConnect("SmartAquariumFeeder", "12345678")) {//Задайте здесь параметры точки доступа (SSID, password)
...
Blynk.virtualWrite(V3, 450); // Задаём максимальную скорость двигателя
Blynk.setProperty(V3, "label", "Максимальная скорость двигателя");
Blynk.virtualWrite(V4, 350); // Задаём ускорение двигателя
Blynk.setProperty(V4, "label", "Ускорение двигателя");
Blynk.virtualWrite(V5, 2000); // Установим скорость в шагах за секунду
Blynk.setProperty(V5, "label", "Скорость в шагах за секунду");

s28BYJ.setMaxSpeed(450);
s28BYJ.setAcceleration(350);
s28BYJ.setSpeed(2000);

Blynk.setProperty(V2, "label", "Вывалиить корм");
Blynk.setProperty(V1, "label", "Втянуть корм в кормушку");
    ...
    // Установим скорость в шагах за секунду
      ...
      if (str=="1")
        {          
          ...
          Blynk.notify("Запущена кормешка рыб");
        }
      if (str=="0")
      {        
        Blynk.virtualWrite ( 2 , 0 );
        Blynk.syncVirtual (V2 );
        Blynk.notify("Кормежка рыб окончена");
      }     
  }
